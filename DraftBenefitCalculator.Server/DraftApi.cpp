...
DraftApi::DraftApi(utility::string_t url) : m_listener(url)
{
	...
	m_listener.support(methods::OPTIONS, std::bind(&DraftApi::handle_options, this, std::placeholders::_1));
...
// Method to handle the OPTIONS requests
void DraftApi::handle_options(http_request message)
{
...
void DraftApi::handle_get(http_request message)
{
	...
	response.headers().add(U("Access-Control-Allow-Origin"), U("*"));
	response.set_body(U("GET method is not supported. Please use POST method to send the parameters."));
	message.reply(response);
}
...
void DraftApi::handle_post(http_request message)
{
	...
	try
	{
		...
		std::cout << "Received request: " << to_utf8(request.serialize()) << "\n";
		double result = calculate_draft_benefit(request);
		std::cout << "Calculated result: " << result << "\n";  // Print the result to the console

		response.set_body(json::value::number(result));
	}
	catch (const std::exception&)
	{
		response.set_status_code(status_codes::BadRequest);
		response.set_body(U("Invalid JSON object. Please check the parameters and the format."));
	}
	message.reply(response);
}
...
void DraftApi::handle_put(http_request message)
{
	http_response response(status_codes::BadRequest);
	response.headers().add(U("Access-Control-Allow-Origin"), U("*"));
	response.set_body(U("PUT method is not supported. Please use POST method to send the parameters."));
	message.reply(response);
}
...
void DraftApi::handle_delete(http_request message)
{
	http_response response(status_codes::BadRequest);
	response.headers().add(U("Access-Control-Allow-Origin"), U("*"));
	response.set_body(U("DELETE method is not supported. Please use POST method to send the parameters."));
	message.reply(response);
}
